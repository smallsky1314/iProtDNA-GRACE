...
def esm2(data):
    ...
    with torch.no_grad():
        results = model(batch_tokens, repr_layers=[48], return_contacts=False)
    token_representations = results["representations"][48].cpu().numpy()

    return token_representations[0, 1: -1]


def main():
    file_path = 'data/DNAPred_Dataset/PDNA-335_sequence.fasta'
    ...
    for record in tqdm(SeqIO.parse(file_path, 'fasta')):
        ...
        matrix = esm2(data)
        np.savetxt('data/335_52/' + record.id + '.csv', matrix, delimiter=',')
